-- Keep the payroll import-history screen summary-first. The batch list needs an
-- aggregate amount so the browser does not fetch every payslip merely to render
-- the history rows. Detailed payslips are still returned only for p_batch_id.
