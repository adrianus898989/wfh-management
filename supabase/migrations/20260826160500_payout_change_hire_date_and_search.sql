-- Keep the payout-change list contract aligned with its visible columns and
-- comprehensive search field. This is a same-signature forward replacement;
-- review, scoping, current-session and manual-fulfillment behavior is unchanged.
...
begin
  ...
  where (v_status = '' or request.status = v_status)
    ...
    and (
      ...
      or exists (
        select 1
        from public.user_access requester_search
        where requester_search.auth_user_id = request.requested_by
          and (
            lower(coalesce(requester_search.login_username,'')) like '%'||v_search||'%'
            or lower(coalesce(requester_search.login_email,'')) like '%'||v_search||'%'
          )
      )
    );
  ...
  from (
    ...
    left join public.teams team on team.id = employee.team_id
    left join public.positions position on position.id = employee.position_id
    left join public.user_access requester on requester.auth_user_id = request.requested_by
    ...
    where (v_status = '' or request.status = v_status)
      ...
      and (
        ...
        or lower(coalesce(employee.employee_no,'')) like '%'||v_search||'%'
        or lower(coalesce(employee.full_name,'')) like '%'||v_search||'%'
        or lower(coalesce(requester.login_username,'')) like '%'||v_search||'%'
        or lower(coalesce(requester.login_email,'')) like '%'||v_search||'%'
        or lower(coalesce(team.name,'')) like '%'||v_search||'%'
        or lower(coalesce(position.name,'')) like '%'||v_search||'%'
        or lower(coalesce(request.reason,'')) like '%'||v_search||'%'
        or lower(coalesce(request.review_note,'')) like '%'||v_search||'%'
...
comment on function public.admin_payout_change_requests(text,text,integer,integer) is
  'Returns scoped payout-change records with employee hire date, comprehensive employee/requester/organization search, and unchanged manual fulfillment status.';
