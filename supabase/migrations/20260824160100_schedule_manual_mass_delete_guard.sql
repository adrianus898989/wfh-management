-- Keep the removal guard active for manual/installation pushes as well as
-- automatic changes. Manual troubleshooting must never turn a formula-column
-- outage into a mass deletion of the employee directory.
...
comment on function public.ingest_schedule_roster_snapshot(jsonb) is
  'Service-only schedule ingest preserving name-only rows and always guarding duplicate/mass-removed IDs.';
