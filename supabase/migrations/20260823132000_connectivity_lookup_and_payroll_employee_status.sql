-- Validate connectivity employee IDs while the form is being filled and expose
-- a stable payroll match classification without giving the browser direct table access.
...
begin
  ...
  if not (public.has_permission('employee.view') or public.has_permission('employee.edit')) then
...
set search_path = ''
as $$
  select employee_ops_private.admin_connectivity_employee_lookup(p_employee_no);
$$;
  ...
  select jsonb_build_object(
    ...
    'batches',coalesce((
      ...
      from (
        select b.id,b.period_start,b.title,b.currency,b.status,b.source_type,b.source_file_name,b.notes,
          ...
          count(p.id) filter(where p.employee_id is not null and coalesce(e.status,'active')='active')::integer active_count,
          count(p.id) filter(where (p.employee_id is not null and coalesce(e.status,'active')<>'active') or (p.employee_id is null and p.departure_date is not null))::integer resigned_count,
    ...
    'rows',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'match_state',case
          when p.employee_id is not null and coalesce(e.status,'active')='active' then 'active'
          when p.employee_id is not null then 'resigned'
