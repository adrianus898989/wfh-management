...
async function orders(service:any,body:any){
  ...
  const [summary,allErrors]=await Promise.all([loadSyncedOrderSummary(service,from,to,accounts),loadAllErrors(service)])
  const activeDates=(summary.dates||[]).map(text).filter(Boolean).sort()
  const activeSet=new Set(activeDates),byId=new Map<string,any>()
  ...
  return {updated_at:new Date().toISOString(),source:'supabase_order_rows',from:from||summary.available_from||'',to:to||summary.available_to||'',dates:activeDates,available_from:summary.available_from||'',available_to:summary.available_to||'',rows,options:{positions:uniq([...allowedIds].map(id=>ctx.rosterById.get(id)?.position)).sort()},sync_state:await syncState(service)}
}
