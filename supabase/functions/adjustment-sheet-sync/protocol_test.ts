...
Deno.test("rejects wrong month and workbook currency", async () => {
  const base = {
    ...
    rows: [{
      ...
      employee_name: "A", note: "ok", google_row: 3,
    }],
  };
...
Deno.test("rejects invalid or duplicate source slots and requires employee ID", async () => {
  const row = {
    ...
    currency: "PHP", employee_no: "PH-12", employee_name: "Ana", note: "ok", google_row: 8,
  };
  const payload = {
    request_id: "24d973ec-7b8e-4a1a-84d4-3ac43de746ea",
    source_key: "adjustment_home_ph_2026_09",
  ...
  await assertRejects(
    () => normalizeInbound({
      ...payload,
      rows: [row, { ...row, external_id: "bd5d7d5d-9f72-42f0-90b2-9484317070f5" }],
    }),
    Error,
  ...
  await assertRejects(
    () => normalizeInbound({
      ...payload,
      rows: [row, { ...row, source_slot: "second_half" }],
    }),
    Error,
