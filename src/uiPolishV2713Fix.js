...
function resetGradeWithPage(e){const b=e.target.closest('button');if(!b||text(b.textContent)!=='重置')return;if(b.closest('.archive-filter-actions'))document.querySelector('.wfh-employee-risk-filter .wfh-grade-menu button')?.click();if(b.closest('.wfh-error-unified'))document.querySelector('.wfh-error-grade-slot .wfh-grade-menu button')?.click()}
...
export function startUiPolishV2713Fix(){if(window.__WFH_UI_POLISH_V2713_FIX__)return;window.__WFH_UI_POLISH_V2713_FIX__=true;document.addEventListener('click',forceArchiveGradeReload,true);document.addEventListener('click',resetGradeWithPage,true);const o=new MutationObserver(schedule);o.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});schedule();window.addEventListener('beforeunload',()=>{stopped=true;o.disconnect();document.removeEventListener('click',forceArchiveGradeReload,true);document.removeEventListener('click',resetGradeWithPage,true)},{once:true})}
