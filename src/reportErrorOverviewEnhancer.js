...
async function load(){
  ...
  loading=true
  try{
    const {data,error}=await supabase.from('employee_error_summary').select('*').order('month_error_count',{ascending:false}).limit(5000)
    if(!error)rows=data||[]
    lastLoaded=Date.now()
...
function render(){
  ...
  const card=old||document.createElement('section');card.className='wfh-eo-card';card.replaceChildren()
  const head=document.createElement('div');head.className='wfh-eo-head';const left=document.createElement('div');const h=document.createElement('h3');h.textContent='员工错误风险总览';const p=document.createElement('p');p.textContent='对应「错误统计」：本月分级 + 近30天 + 累计错误；点击 TOP 员工ID可直接跳到该员工错误记录。';left.append(h,p);const stamp=document.createElement('span');stamp.textContent=`${monthText(month)} · 自动刷新`;head.append(left,stamp)
  const kp=document.createElement('div');kp.className='wfh-eo-kpis';kp.append(kpi('本月错误总笔数',monthErrors,'员工错误合计'),kpi('近30天错误',last30,'滚动30天合计'),kpi('有错误员工',withErrors,`共 ${monthRows.length} 名有统计`),kpi('高频员工',counts.high,'本月 ≥10 笔','high'),kpi('重点观察',counts.watch,'本月 6–9 笔','watch'),kpi('注意员工',counts.attention,'本月 4–5 笔','attention'))
