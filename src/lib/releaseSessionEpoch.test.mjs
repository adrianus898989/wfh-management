...
test('legacy GitHub Pages publishes only a Cloudflare redirect and cannot rotate live app sessions', () => {
  ...
  assert.match(workflow, /LEGACY_REDIRECT_RELEASE_ID: \$\{\{ github\.sha \}\}:\$\{\{ github\.run_id \}\}:\$\{\{ github\.run_attempt \}\}:redirect-only/)
  assert.doesNotMatch(workflow, /npm run build(?:\s|$)/)
  assert.doesNotMatch(workflow, /VITE_SUPABASE|SUPABASE_SERVICE_ROLE_KEY|app_release_advance|VITE_APP_RELEASE_ID|^\s+APP_RELEASE_ID:/m)
  ...
  assert.match(githubRedirectBuilder, /resolve\(outputDirectory, 'release\.json'\)/)
  assert.match(githubRedirectBuilder, /JSON\.stringify\(\{ releaseId \}\)/)
  assert.doesNotMatch(githubRedirectBuilder, /VITE_SUPABASE|src\/main/)
