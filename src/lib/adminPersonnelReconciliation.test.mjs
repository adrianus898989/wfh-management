...
test('reason and onsite labels clearly separate accepted staff from actionable differences', () => {
  ...
  assert.equal(personnelReconciliationIssueLabel({ issue_code:'pending_manual_review' }), '来源状态待确认（不直接计入人数误差）')
  assert.equal(personnelReconciliationReasonLabel({ reason_code:'missing_from_both_complete_sources' }), '后台主档仍为在职，但两份当前来源未确认到同一人（系统未自动改状态）')
  assert.equal(personnelReconciliationReasonLabel({ reason_code:'future_resignation_removed_from_schedule_early' }), '离职日期尚未生效，但已提前从当前排班移除')
  assert.equal(personnelReconciliationReasonLabel({ reason_code:'home_source_resigned_profile_still_active' }), '居家名单已标离职，但后台主档仍为在职')
...
test('standalone page calls one server-paged RPC and preserves old rows while refreshing', () => {
  ...
  assert.match(page, /RECONCILIATION_REFRESH_MS = 5 \* 60 \* 1000/)
  assert.match(page, /document\.visibilityState !== 'visible'/)
  assert.match(page, /if \(controllerRefs\.current\[context\.view\]\) return/)
  assert.match(page, /Date\.now\(\) - lastCompletedAt < RECONCILIATION_REFRESH_MS/)
  assert.match(page, /window\.setInterval\(refreshVisibleView, RECONCILIATION_REFRESH_MS\)/)
  assert.match(page, /document\.addEventListener\('visibilitychange', refreshVisibleView\)/)
  assert.match(page, /window\.clearInterval\(refreshTimer\)/)
  assert.match(page, /document\.removeEventListener\('visibilitychange', refreshVisibleView\)/)
  ...
  assert.match(page, /void load\(\{[\s\S]*?view,[\s\S]*?page:1,[\s\S]*?size:pageSize,[\s\S]*?search:appliedSearch/)
  ...
  assert.match(page, /员工档案当前目录（来源）/)
  assert.match(page, /来源原始人数，可能含未来入职或待移除记录/)
  ...
  assert.match(page, /页面每 5 分钟自动读取/)
  ...
  assert.match(page, /未来入职不计入/)
  assert.match(page, /只保留后台历史档案，不再算误差/)
  assert.match(page, /这里是来源提示，不等于人数误差/)
...
test('page styles stay scoped and retain a compact responsive table', () => {
  ...
  assert.match(styles, /\.recon-policy-note/)
