...
test('standalone page calls one server-paged RPC and preserves old rows while refreshing', () => {
  ...
  assert.match(page, /return Boolean\(controllerRefs\.current\[context\.view\]\)/)
  assert.match(page, /return lastCompletedAtRef\.current\[context\.view\] \|\| 0/)
  assert.match(page, /return loadRef\.current\?\.\(context\)/)
