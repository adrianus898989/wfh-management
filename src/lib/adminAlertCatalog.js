...
export const ADMIN_ALERT_TYPES = {
  ...
  today_missing_daily_report: {
    ...
    permissions:['daily_work.manage','report.view'], ready:false,
  ...
  late_timeout_frequency: {
    ...
    permissions:['adjustment.view'], ready:true,
  },
  consecutive_rest: {
    zh:'连续公休', en:'Consecutive rest', icon:'休', tone:'violet', group:'attendance',
    permissions:['attendance.view'], ready:true,
  },
  weekly_absence: {
  ...
  error_spike: {
    ...
    permissions:['report.view'], ready:true,
  },
  repeated_error: {
    zh:'重复错误', en:'Repeated errors', icon:'重', tone:'slate', group:'quality',
    permissions:['report.view'], ready:false,
  ...
  exam_failed: {
    ...
    permissions:['exam.view'], ready:true,
  },
  low_workload_streak: {
...
export function canViewAdminAlertType(access, type) {
  ...
  return Boolean(meta && meta.permissions.some(code => adminAlertHasPermission(access, code)))
}
