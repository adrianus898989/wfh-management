...
test('online status reuses the existing low-frequency application lease', () => {
  ...
  assert.match(component, /人数约每 3 分钟更新；名单只在展开时读取；连续 5 分钟没有心跳即显示离线/)
})
...
test('presence keeps background refresh count-only and loads one authorised page only on demand', () => {
  ...
  assert.match(component, /loadRows\(value, \{ page:1 \}\)/)
  ...
  assert.match(component, /onClick=\{\(\) => loadRows\(section, \{ page:selectedPresence\.page \}\)\}/)
