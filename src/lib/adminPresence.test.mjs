...
test('online status reuses the existing low-frequency application lease', () => {
  ...
  assert.match(endpoint, /userClient[\s\S]*\.rpc\('admin_online_presence_allowed'\)/)
...
test('online presence scopes only live employees without loading the full account directory', () => {
  ...
  assert.match(source, /!isFounder && activeCaller\.data_scope !== 'all'/)
  assert.match(source, /callerDataScope === 'self'[\s\S]*activeCaller\.employee_id/)
  assert.match(source, /\['own_team', 'assigned_teams'\]\.includes\(callerDataScope\)/)
  assert.match(source, /from\('user_scope_employees'\)[\s\S]*\.eq\('auth_user_id', authenticatedUser\.id\)[\s\S]*\.in\('employee_id', liveEmployeeIds\)/)
  assert.match(source, /new Set\(\(effectiveRows \|\| \[\]\)/)
  assert.match(source, /\.in\('id', \[\.\.\.visibleEmployeeIds\]\)/)
  assert.match(source, /account\.employee_id[\s\S]*\? canSeeEmployee\(account\.employee_id\)[\s\S]*: isFounder/)
  assert.doesNotMatch(source, /: \(isFounder \|\| activeCaller\.data_scope === 'all'\)/)
