...
test('daily-report delete confirmation preserves its parent until the archive succeeds',()=>{
  ...
  assert.match(archiveSource,/const reason=reportArchived/)
  assert.match(archiveSource,/if\(reportArchived\)setError\(reason\)\n\s+else setDeleteError\(reason\)/)
  assert.match(archiveSource,/operation:reportArchived\?'删除后刷新日报列表':'删除线上培训日报'/)
  assert.match(archiveSource,/retry:\(\)=>loadList\(\{silent:true,announceFailure:true,operation:'刷新确认删除结果'\}\)/)
  assert.doesNotMatch(archiveSource,/retry:[^\n]*archiveReport/)
