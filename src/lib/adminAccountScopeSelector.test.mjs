...
test('account search keeps four explicit fields alongside adjacent actions', () => {
  assert.match(usersPage, /\.access-searchbar\{display:grid;grid-template-columns:repeat\(4,minmax\(140px,1fr\)\) auto auto auto;/)
  assert.match(usersPage, /\.access-searchbar\{[^}]+justify-content:start/)
  assert.match(usersPage, /输入后台用户名[\s\S]+输入员工ID或姓名[\s\S]+输入角色、管理范围或创建人[\s\S]+账号状态/)
})
