...
test('routine auth and wake events do not repeat a fresh IP claim', () => {
  ...
  assert.match(app, /const onVisibilityChanged = \(\) => \{[\s\S]{0,180}?if \(document\.hidden\)[\s\S]{0,180}?recover\(\{ skipIfFresh:true \}\)/)
  assert.match(app, /const onFocus = \(\) => \{[\s\S]{0,100}?recover\(\{ skipIfFresh:true \}\)[\s\S]{0,100}?scheduleHeartbeat\(\)/)
