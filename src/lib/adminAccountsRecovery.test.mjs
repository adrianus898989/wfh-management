...
test('recovery staff account tab keeps its bounded reader separate from the single-account delete action', async () => {
  ...
  const mutationStart = edge.indexOf("if (action === 'unlock_staff_login')", staffStart)
  const staffSource = edge.slice(staffStart, mutationStart)
  ...
  assert.match(page, /员工前端账号支持受权限分页查看、搜索、重置员工登录密码，以及逐个删除登录账号/)
...
test('recovery staff deletion is exact, staff-only, fail-closed and outcome-reconciled', async () => {
  const edge = await read('../../supabase/functions/admin-accounts/recovery.ts')
  const migration = await read('../../supabase/migrations/20260904083345_staff_account_retention_soft_delete.sql')
  ...
  assert.match(edge, /const RECOVERY_STAFF_ACCOUNT_ACTIONS = \[[\s\S]+delete_staff_account[\s\S]+unlock_staff_login[\s\S]+reset_staff_password/)
  assert.match(edge, /const RECOVERY_STAFF_ACCOUNT_ACTION_PERMISSION:[\s\S]+delete_staff_account:'user\.account\.delete',[\s\S]+unlock_staff_login:'staff_account\.unlock',[\s\S]+reset_staff_password:'user\.password\.reset'/)
  ...
  const softDelete = deletion.indexOf('admin.auth.admin.deleteUser(targetAuthUserId, true)', reprepare)
  const finalize = deletion.indexOf('admin_recovery_finalize_staff_account_delete_v1', softDelete)
  assert.ok(prepare > 0 && exactAuthRead > prepare && secondGate > exactAuthRead)
  assert.ok(reprepare > secondGate && softDelete > reprepare && finalize > softDelete)
  ...
  assert.match(deletion, /staff_auth_identity_still_active/)
  assert.doesNotMatch(deletion, /deleteUser\(targetAuthUserId, false\)/)
  assert.doesNotMatch(edge.match(/const RECOVERY_ACCOUNT_ACTIONS = \[[\s\S]*?\]/)?.[0] || '', /delete/)

  const tableMigration = await read('../../supabase/migrations/20260902120253_recovery_staff_account_safe_delete.sql')
  const tableStart = tableMigration.indexOf('create table if not exists scope_private.recovery_staff_account_delete_operations')
  const tableEnd = tableMigration.indexOf('create or replace function public.admin_recovery_prepare_staff_account_delete_v1')
  ...
  const tableSource = tableMigration.slice(tableStart, tableEnd)
  ...
  assert.match(prepareSource, /login_admin_permission_allowed\([\s\S]+staff_account\.view[\s\S]+login_admin_permission_allowed\([\s\S]+user\.account\.delete/)
  ...
  assert.match(prepareSource, /recovery_staff_storage_is_safely_retained/)
  const storageGuard = prepareSource.indexOf('staff_account_owns_unretained_storage_objects')
  ...
  assert.match(finalizeSource, /auth_user\.deleted_at is not null/)
  assert.match(finalizeSource, /delete from public\.user_access target/)
  assert.match(finalizeSource, /set status = 'expired'/)
  ...
  assert.match(finalizeSource, /'exam_history_retained', true/)
  assert.match(finalizeSource, /'referenced_employee_files_retained', true/)
