...
export function AdminPayoutChangeWorkspace({ mode = 'pending', canReview = false }) {
  ...
  return <section className="admin-payment-change">
    ...
    <div className="admin-payment-change-table"><table><thead><tr><th>{adminT('申请人 / 时间')}</th><th>{adminT('员工')}</th><th>{adminT('员工类型 / 国家')}</th><th>{adminT('团队 / 岗位')}</th><th>{adminT('修改项目')}</th><th>{adminT('原因')}</th><th>{adminT('审核结果')}</th><th>{adminT('实际资料处理')}</th><th>{adminT('操作')}</th></tr></thead><tbody>{state.loading ? <tr><td colSpan="9"><div className="payment-change-empty">{adminT('正在读取申请…')}</div></td></tr> : rows.length ? rows.map(row => <tr key={row.id}><td><b>{row.requested_by || adminT('员工本人')}</b><span>{dateTime(row.created_at, locale)}</span></td><td><b>{row.employee_no}</b><span>{row.employee_name}</span></td><td><b>{row.employment_type || '—'}</b><span>{row.country || '—'} · {row.employee_status || '—'}</span></td><td><b>{row.team_name || '—'}</b><span>{row.position_name || '—'}</span></td><td>{adminT(row.payment_kind === 'bank_wallet' ? '银行卡 / 钱包' : 'USDT 地址')}</td><td className="admin-payment-change-reason"><span title={row.reason}>{row.reason}</span></td><td><i className={`payment-change-status ${row.status}`}>{statusLabel(row.status, adminCopy)}</i><span>{row.reviewed_at ? `${row.reviewed_by || adminT('后台账号')} · ${dateTime(row.reviewed_at, locale)}` : adminT('尚未审核')}</span></td><td><i className={`payment-change-fulfillment-pill ${fulfillmentTone(row.fulfillment_status)}`}>{adminT(fulfillmentLabel(row.fulfillment_status, adminCopy))}</i><span>{row.fulfilled_at ? `${row.fulfilled_by || adminT('系统 / 外部同步')} · ${dateTime(row.fulfilled_at, locale)}` : adminT('自动修改已关闭')}</span></td><td><div className="admin-payment-change-actions"><button type="button" onClick={() => setDetail(row)}>{adminT('详情')}</button>{row.status === 'pending' && canReview && <button className="primary" type="button" onClick={() => { setMessage(''); setReview({ ...row, decision: 'approved', note: '' }) }}>{adminT('审核')}</button>}</div></td></tr>) : <tr><td colSpan="9"><div className="payment-change-empty">{adminT('暂无符合条件的申请。')}</div></td></tr>}</tbody></table></div>
