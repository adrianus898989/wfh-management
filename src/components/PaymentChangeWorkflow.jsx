...
const COPY = {
  zh: {
    ...
    pendingExists: '已有一笔申请等待审核，审核完成前不能重复提交。', old: '旧资料（必须完全一致）', next: '新资料',
  ...
  en: {
    ...
    pendingExists: 'One request is already pending. You can submit another after it is reviewed.', old: 'Current details (must match exactly)', next: 'New details',
  ...
  vi: {
    ...
    pendingExists: 'Bạn đã có một yêu cầu đang chờ duyệt.', old: 'Thông tin cũ (phải khớp hoàn toàn)', next: 'Thông tin mới',
  ...
  id: {
    ...
    pendingExists: 'Sudah ada satu permintaan yang menunggu persetujuan.', old: 'Data lama (harus sama persis)', next: 'Data baru',
...
export function AdminPayoutChangeWorkspace({ mode = 'pending', canReview = false }) {
  ...
  const decide = async (decision, note = '') => {
    ...
    setMessage(adminT(decision === 'approved' ? '申请已批准，员工收款资料已安全更新。' : '申请已驳回，员工可在前端查看原因。'))
  ...
  return <section className="admin-payment-change">
    <header className="admin-payment-change-head"><div><small>PAYMENT CHANGE REVIEW</small><h2>{adminT(mode === 'pending' ? '收款资料审核' : '收款资料申请记录')}</h2><p>{adminT('证明文件保存在私有空间；只有具备权限且在管理范围内的账号可以查看。')}</p></div><strong>{locale === 'en' ? `${Number(state.data?.total || 0).toLocaleString()} records` : `${Number(state.data?.total || 0).toLocaleString()} 条`}</strong></header>
    <form className="admin-payment-change-filters" onSubmit={submitSearch}><label><span>{adminT('员工 / 原因')}</span><input value={filters.search} onChange={event => setFilters({ ...filters, search: event.target.value })} placeholder={adminT('员工ID、姓名或修改原因')} /></label>{mode !== 'pending' && <label><span>{adminT('状态')}</span><select value={filters.status} onChange={event => { setPage(1); setFilters({ ...filters, status: event.target.value }) }}><option value="">{adminT('全部状态')}</option><option value="approved">{adminT('已通过')}</option><option value="rejected">{adminT('已驳回')}</option><option value="pending">{adminT('待审核')}</option><option value="cancelled">{adminT('已取消')}</option></select></label>}<button className="primary" type="submit">{adminT('查询')}</button><button type="button" onClick={resetFilters}>{adminT('重置')}</button></form>
    ...
    <div className="admin-payment-change-table"><table><thead><tr><th>{adminT('提交时间')}</th><th>{adminT('员工')}</th><th>{adminT('员工类型 / 国家')}</th><th>{adminT('团队 / 岗位')}</th><th>{adminT('修改项目')}</th><th>{adminT('原因')}</th><th>{adminT('状态')}</th><th>{adminT('操作')}</th></tr></thead><tbody>{state.loading ? <tr><td colSpan="8"><div className="payment-change-empty">{adminT('正在读取申请…')}</div></td></tr> : rows.length ? rows.map(row => <tr key={row.id}><td>{dateTime(row.created_at, locale)}</td><td><b>{row.employee_no}</b><span>{row.employee_name}</span></td><td><b>{row.employment_type || '—'}</b><span>{row.country || '—'} · {row.employee_status || '—'}</span></td><td><b>{row.team_name || '—'}</b><span>{row.position_name || '—'}</span></td><td>{adminT(row.payment_kind === 'bank_wallet' ? '银行卡 / 钱包' : 'USDT 地址')}</td><td className="admin-payment-change-reason"><span title={row.reason}>{row.reason}</span></td><td><i className={`payment-change-status ${row.status}`}>{statusLabel(row.status, adminCopy)}</i></td><td><div className="admin-payment-change-actions"><button type="button" onClick={() => setDetail(row)}>{adminT('详情')}</button>{row.status === 'pending' && canReview && <button className="primary" type="button" onClick={() => { setMessage(''); setReview({ ...row, decision: 'approved', note: '' }) }}>{adminT('审核')}</button>}</div></td></tr>) : <tr><td colSpan="8"><div className="payment-change-empty">{adminT('暂无符合条件的申请。')}</div></td></tr>}</tbody></table></div>
...
function PaymentChangeDetail({ request, canReview, onClose, onReview }) {
  ...
  const openProof = async (path, label) => {
    if (!clean(path)) {
      setPreview({ loading: false, error: adminT('该历史申请没有保存此证明文件。'), url: '', label, path: '' })
      return
    }
    setPreview({ loading: true, error: '', url: '', label, path })
    const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(path, 120)
    setPreview(error ? { loading: false, error: adminT(requestError(error)), url: '', label, path } : { loading: false, error: '', url: data?.signedUrl || '', label, path })
  }
  const isPdf = clean(preview?.path).toLowerCase().endsWith('.pdf')
  return <div className="payment-change-modal-backdrop" role="presentation" onMouseDown={onClose}><section className="payment-change-modal detail" role="dialog" aria-modal="true" onMouseDown={event => event.stopPropagation()}><header><div><small>PAYMENT CHANGE DETAIL</small><h2>{request.employee_no} · {request.employee_name}</h2></div><button onClick={onClose}>×</button></header><div className="payment-change-detail-grid"><div><span>{adminT('员工')}</span><strong>{request.employee_no} · {request.employee_name}</strong><small>{request.employment_type || '—'} · {request.country || '—'}</small></div><div><span>{adminT('组织')}</span><strong>{request.team_name || '—'} · {request.position_name || '—'}</strong><small>{request.employee_status || '—'}</small></div><div><span>{adminT('提交时间')}</span><strong>{dateTime(request.created_at, locale)}</strong><small>{request.requested_by || adminT('员工本人')}</small></div><div><span>{adminT('状态')}</span><i className={`payment-change-status ${request.status}`}>{statusLabel(request.status, adminCopy)}</i><small>{request.reviewed_at ? dateTime(request.reviewed_at, locale) : '—'}</small></div></div><div className="payment-change-compare"><div><small>{adminT('旧资料')}</small><PaymentFacts kind={request.payment_kind} value={request.old_data} /></div><span>→</span><div><small>{adminT('新资料')}</small><PaymentFacts kind={request.payment_kind} value={request.new_data} /></div></div><div className="payment-change-detail-reason"><small>{adminT('修改原因')}</small><p>{request.reason}</p></div>{request.review_note && <div className="payment-change-reject-note"><b>{adminT('审核备注')}</b><span>{request.review_note}</span></div>}<div className="payment-change-proof-actions"><button onClick={() => openProof(request.identity_proof_path, adminT('身份证明'))}>{adminT('查看身份证明')}</button><button onClick={() => openProof(request.payment_proof_path, adminT('新收款资料截图'))}>{adminT('查看收款截图')}</button></div>{preview && <div className="payment-change-proof-preview"><header><b>{preview.label}</b><button onClick={() => setPreview(null)}>×</button></header>{preview.loading ? <div>{adminT('正在安全读取文件…')}</div> : preview.error ? <div className="payment-change-alert error">{preview.error}<button onClick={() => openProof(preview.path, preview.label)}>{adminT('重试')}</button></div> : isPdf ? <iframe title={preview.label} src={preview.url} /> : <img src={preview.url} alt={preview.label} />}</div>}<footer><button onClick={onClose}>{locale === 'en' ? 'Close' : '关闭'}</button>{request.status === 'pending' && canReview && <button className="primary" onClick={onReview}>{adminT('开始审核')}</button>}</footer></section></div>
}
