...
export default function StaffExamPage(){
  ...
  return <div className="staff-exam-page">
    ...
    {loading?<div className="exam-empty">正在读取考试…</div>:<>
      ...
      <section className="staff-pending-section"><div className="staff-section-head"><div><small>MY POSITION EXAM</small><h2>我的岗位考试</h2><p>系统按你的员工档案自动匹配题库，每次随机抽取 14 题，共 100 分。</p></div><span>{pending.length} 项</span></div><div className="assignment-grid">{pending.map(a=><article key={a.id}><div className="assignment-card-top"><span>{a.resume_session_id?'进行中':'档案已匹配'}</span><b>{a.duration_minutes} 分钟</b></div><h3>{a.title}</h3><p>员工范围：{a.team_name} <i>·</i> {a.position_name}</p><p className="question-scope">题库范围：{a.question_team} <i>·</i> {a.question_position}</p><div className="assignment-facts"><b><small>试卷</small>{a.question_count} 题 / {a.total_score} 分</b><b><small>及格分</small>{a.pass_score}%</b><b><small>已考／上限</small>{a.attempts} / {a.max_attempts}</b></div>{!a.pool_ready&&<div className="pool-warning">题库暂未达到 14 题 / 100 分，请联系管理员补充本岗位题目。</div>}<button disabled={!a.pool_ready||(!a.resume_session_id&&a.attempts>=a.max_attempts)} onClick={()=>start(!!a.resume_session_id)}>{a.resume_session_id?'继续答题 →':!a.pool_ready?'题库准备中':a.attempts>=a.max_attempts?'次数已用完':'开始随机考试 →'}</button></article>)}{!pending.length&&<div className="staff-empty-state"><span>!</span><h3>暂时无法生成岗位考试</h3><p>你的档案是“{home?.profile?.team_name||'未设置'} · {home?.profile?.position_name||'未设置'}”。请管理员为这个范围配置对应题库。</p></div>}</div></section>
