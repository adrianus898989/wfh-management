...
export default function StaffExamPage(){
  ...
  return <div className="staff-exam-page">
    ...
    {loading?<div className="exam-empty">正在读取考试…</div>:<>
      ...
      <section><div className="staff-section-head"><div><small>MY RESULTS</small><h2>我的考试结果</h2><p>只有本人能够查看自己的答案、得分及老师评语。</p></div><span>{history.length} 次</span></div>{history.length?<div className="exam-table-wrap"><table className="exam-table staff-history-table"><thead><tr><th>考试</th><th>次数</th><th>提交时间</th><th>评分完成时间</th><th>成绩</th><th>答题结果</th><th>评分人</th><th>结果</th><th>操作</th></tr></thead><tbody>{history.map(x=><tr key={x.id}><td><strong>{x.title}</strong></td><td>第 {x.attempt_no} 次</td><td>{fmt(x.submitted_at)}</td><td>{fmt(x.graded_at)}</td><td><b>{x.percentage==null?'待批改':`${score(x.earned_score)}/${score(x.total_score)} · ${score(x.percentage)}%`}</b></td><td><span className="staff-history-breakdown">{breakdown(x)}</span></td><td>{x.grader_name||'—'}</td><td><span className={`result-chip ${x.status==='graded'?(x.passed?'pass':'fail'):'pending'}`}>{x.status==='graded'?(x.passed?'通过':'未通过'):'待批改'}</span></td><td><button onClick={()=>viewResult(x.id)}>查看结果</button></td></tr>)}</tbody></table></div>:<div className="staff-history-empty">完成考试后，成绩与历史记录会显示在这里。</div>}</section>
