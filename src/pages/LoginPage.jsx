...
export default function LoginPage(){
  ...
  const submit=async e=>{e.preventDefault();setError('');if(!configured)return setError('Supabase 尚未连接');setLoading(true)
    ...
    if(access.backend_enabled)navigate(publicPortalTarget('admin')); else if(access.employee_portal_enabled)navigate(publicPortalTarget('staff')); else setError('账号未开启入口')
  }
