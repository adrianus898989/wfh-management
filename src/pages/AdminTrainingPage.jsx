...
function Overview({counts,data,onTab,onEmployee}){
  ...
  return <><div className="exam-metrics">{cards.map(([l,v,t])=><button key={l} onClick={()=>onTab(t)}><span>{l}</span><strong>{v}</strong><small>查看详情 →</small></button>)}</div><section className="exam-source-summary"><div><span>本系统考试</span><b>{summary.current_attempts||0}</b><small>当前前端产生的记录</small></div><div className="legacy"><span>旧考试</span><b>{old.total_sessions||0}</b><small>已评分 {old.completed||0} · 待评分 {old.pending_grading||0} · 答题中 {old.in_progress||0}</small></div><div><span>员工匹配</span><b>{old.matched||0}</b><small>未匹配 {old.unmatched||0} 条仍可按原账号查看</small></div><div><span>自动同步</span><b>{sync.status==='success'?'正常':'检查中'}</b><small>{sync.last_success_at?`最近 ${fmt(sync.last_success_at)}`:'等待首次增量检查'}</small></div></section><div className="exam-two"><section className="exam-panel"><h2>最近考试</h2><Sessions rows={(data?.sessions||[]).slice(0,8)} compact onEmployee={onEmployee}/></section><section className="exam-panel adaptive-rule-panel"><h2>员工自适应考试规则</h2><div><b>仅匹配团队</b><span>员工自行选择岗位与盘口</span></div><div><b>14 题 · 100 分</b><span>10×5分＋3×10分＋1×20分</span></div><div><b>60 分钟</b><span>开始后连续计时，答案自动保存</span></div></section></div><ExamAnalytics analytics={analytics} onEmployee={onEmployee}/></>
}
