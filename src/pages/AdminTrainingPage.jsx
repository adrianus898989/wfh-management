...
const blankSessionFilters={employeeNo:'',employeeName:'',exam:'',team:'',position:'',status:'',grader:'',dateFrom:'',dateTo:''}
...
export default function AdminTrainingPage(){
  ...
  const load=async()=>{
    ...
    const [dashboard,analytics]=await Promise.all([
      ...
      supabase.rpc('admin_exam_analytics_v2')
    ])
    if(dashboard.error)setError(message(dashboard.error)); else setData({...dashboard.data,analytics:analytics.data||dashboard.data?.analytics})
    setLoading(false)
  }
  ...
  const loadSessions=async()=>{
    ...
    const {data:result,error:e}=await supabase.rpc('admin_exam_sessions_search_v2',{p_employee_no:sessionFilters.employeeNo,p_employee_name:sessionFilters.employeeName,p_exam:sessionFilters.exam,p_team:sessionFilters.team,p_position:sessionFilters.position,p_status:forcedStatus,p_grader:sessionFilters.grader,p_date_from:sessionFilters.dateFrom||null,p_date_to:sessionFilters.dateTo||null,p_page:sessionPage,p_page_size:sessionPageSize})
  ...
  const openEmployee=s=>navigate(`/admin/employees?tab=${encodeURIComponent('员工档案')}&employee=${encodeURIComponent(s.employee_id)}`)
  const counts=data?.counts||{}

  return <div className="exam-page">
    <header className="exam-head"><div><small>EXAM MANAGEMENT</small><h1>考试管理</h1></div><div className="exam-head-actions"><span className="exam-sync-pill">Google 题库 · {data?.last_sync?.status==='success'?'已同步':'等待同步'}</span><button onClick={load}>刷新</button></div></header>
...
function Overview({counts,data,onTab,onEmployee}){
  ...
  return <><div className="exam-metrics">{cards.map(([l,v,t])=><button key={l} onClick={()=>onTab(t)}><span>{l}</span><strong>{v}</strong><small>查看详情 →</small></button>)}</div><div className="exam-two"><section className="exam-panel"><h2>最近考试</h2><Sessions rows={(data?.sessions||[]).slice(0,8)} compact onEmployee={onEmployee}/></section><section className="exam-panel adaptive-rule-panel"><h2>员工自适应考试规则</h2><div><b>仅匹配团队</b><span>员工自行选择岗位与盘口</span></div><div><b>14 题 · 100 分</b><span>10×5分＋3×10分＋1×20分</span></div><div><b>60 分钟</b><span>开始后连续计时，答案自动保存</span></div></section></div><ExamAnalytics analytics={analytics} onEmployee={onEmployee}/></>
}
...
function Leaderboard({rows,onEmployee}){
  ...
  return <div className="exam-leaderboard"><header><div><h3>考试排行榜</h3><p>按平均分、最高分和考试次数综合排序</p></div><span>TOP {visible.length}</span></header>{visible.length?<div className="exam-leaderboard-list">{visible.map(row=><button key={row.employee_id} onClick={()=>onEmployee?.(row)}><b className={`rank r${row.rank}`}>{row.rank<=3?['🥇','🥈','🥉'][row.rank-1]:row.rank}</b><span><strong>{row.employee_name}</strong><small>{row.employee_no} · {row.team_name}</small></span><em>{score(row.average_score)}<small>平均分</small></em><em>{score(row.best_score)}<small>最高分</small></em><em>{row.attempts}<small>次数</small></em><i>查看记录 →</i></button>)}</div>:<div className="exam-empty compact">暂无排行榜数据</div>}</div>
}
...
function GradeModal({session,onClose,onChanged}){
  ...
  return <Modal title={`考试答卷 · ${session.employee_name} · 第 ${session.attempt_no} 次`} onClose={onClose} wide><div className="grade-body">{error&&<div className="exam-error">{error}</div>}{detail?.session&&<><div className="grade-summary"><span>{s.employee_no}</span><span>{s.team_name} · {s.position_name}</span><span>第 {s.attempt_no} 次</span><span>{statusText(s.status)}</span><span>{s.percentage==null?'待完成评分':`${score(s.earned_score)}/${score(s.total_score)} · ${score(s.percentage)}%`}</span></div><div className="grade-audit-grid"><span><small>正确</small><b>{s.correct_count||0} 题</b></span><span><small>半对</small><b>{s.partial_count||0} 题</b></span><span><small>错误</small><b>{s.wrong_count||0} 题</b></span><span><small>待评分</small><b>{s.pending_count||0} 题</b></span><span><small>开始作答时间</small><b>{fmt(s.started_at)}</b></span><span><small>完成作答时间</small><b>{fmt(s.submitted_at)}</b></span><span><small>评分完成时间</small><b>{fmt(s.graded_at)}</b></span><span><small>评分人</small><b>{s.grader_name||'—'}</b></span></div></>}{!detail?<div className="exam-empty">读取答卷中…</div>:(detail.answers||[]).map((a,i)=><article className="grade-item" key={a.question_id}><header><b>{i+1}</b><strong>{a.question_zh||a.question_en||a.question_vi}</strong><span className={`grade-score-pill ${a.grade_status||'pending'}`}>{a.awarded_score==null?`待评分 / ${score(a.points)} 分`:`${score(a.awarded_score)}/${score(a.points)} 分`}</span></header><details className="grade-translations"><summary>查看英文 / 越南文题目</summary>{a.question_en&&<p><b>EN</b>{a.question_en}</p>}{a.question_vi&&<p><b>VI</b>{a.question_vi}</p>}</details>{a.image_urls?.length>0&&<div className="question-images">{a.image_urls.map(u=><a href={u} target="_blank" rel="noreferrer" key={u}><img src={u}/></a>)}</div>}<div className="answer-box"><small>员工答案</small><p>{a.answer_text||'未作答'}</p></div><label className="grade-feedback">老师评语<textarea value={drafts[a.answer_id]?.feedback||''} onChange={e=>setDrafts({...drafts,[a.answer_id]:{...drafts[a.answer_id],feedback:e.target.value}})} placeholder="填写错误原因、正确处理方式或复训要求"/></label>{a.graded_at&&<div className="grade-item-audit">本题评分：{a.grader_name||'—'} · {fmt(a.graded_at)}</div>}<div className="grade-actions"><button className={a.grade_status==='wrong'?'picked':''} disabled={busy===a.answer_id} onClick={()=>grade(a,'wrong',0)}>错误 · 0/{score(a.points)}</button><button className={a.grade_status==='partial'?'picked':''} disabled={busy===a.answer_id} onClick={()=>grade(a,'partial',a.points/2)}>半对 · {score(a.points/2)}/{score(a.points)}</button><button className={a.grade_status==='correct'?'picked':''} disabled={busy===a.answer_id} onClick={()=>grade(a,'correct',a.points)}>正确 · {score(a.points)}/{score(a.points)}</button></div></article>)}</div><footer><button className="primary" onClick={onClose}>完成并关闭</button></footer></Modal>
}
