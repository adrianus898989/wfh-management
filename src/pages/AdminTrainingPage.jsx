...
const breakdown=x=>x.source_system==='legacy'?'旧系统未提供可靠逐题判定':`正确 ${x.correct_count||0} · 半对 ${x.partial_count||0} · 错误 ${x.wrong_count||0}`
...
export default function AdminTrainingPage(){
  ...
  return <div className="exam-page">
    <header className="exam-head"><div><small>EXAM MANAGEMENT</small><h1>考试管理</h1></div><div className="exam-head-actions"><span className="exam-sync-pill">Google 题库 · {data?.last_sync?.status==='success'?'已同步':'等待同步'}</span><span className={`exam-sync-pill legacy ${data?.legacy?.sync_state?.status||''}`}>旧考试 · {data?.legacy?.sync_state?.status==='success'?'自动同步正常':'同步检查中'}</span><button onClick={load}>刷新</button></div></header>
...
function Overview({counts,data,onTab,onEmployee}){
  ...
  const cards=[
    ...
    ['本系统',summary.current_attempts||0,null,'份'],['旧考试',old.total_sessions||0,null,`已评 ${old.completed||0} · 待评 ${old.pending_grading||0}`],['已匹配',old.matched||0,null,`未匹配 ${old.unmatched||0}`],['同步',sync.status==='success'?'正常':'检查中',null,sync.last_success_at?fmt(sync.last_success_at):'等待首次同步']
  ]
  return <><section className="exam-overview-strip">{cards.map(([label,value,target,note],index)=>{const content=<><span>{label}</span><strong>{value}</strong><small>{note}{target?' · 查看 →':''}</small></>;return target?<button key={label} onClick={()=>onTab(target)}>{content}</button>:<div key={label} className={index===5?'legacy':''}>{content}</div>})}</section><div className="exam-two exam-overview-lower"><section className="exam-panel exam-recent-panel"><div className="exam-section-title"><h2>最近考试</h2><button onClick={()=>onTab('考试记录')}>查看全部</button></div><Sessions rows={(data?.sessions||[]).slice(0,5)} compact onEmployee={onEmployee}/></section><section className="exam-panel adaptive-rule-panel"><h2>考试规则</h2><div><b>仅匹配团队</b><span>员工自行选择岗位与盘口</span></div><div><b>14 题 · 100 分</b><span>10×5分＋3×10分＋1×20分</span></div><div><b>60 分钟</b><span>连续计时 · 自动保存</span></div></section></div><ExamAnalytics analytics={analytics} onEmployee={onEmployee}/></>
}
