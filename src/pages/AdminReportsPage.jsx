...
export default function AdminReportsPage(){
  ...
  const syncOrders=overview?.sync_state?.['效率表/网站数据']
  ...
  return <div className="content-page reports-page rp-page">
    ...
    {tab!=='错误统计'&&<GlobalFilters value={draftFilters} onChange={setDraftFilters} onQuery={()=>setFilters({...draftFilters})} onReset={()=>{const next=blankFilters();setDraftFilters(next);setFilters(next)}} options={overview?.options||{}} meta={`${overview?.roster?.length||0} 行 · 当前筛选 ${roster.length} 行 · ${uniqueCount(roster)} 人`}/>} 
    ...
    {loading&&!overview?<Loading/>:<>
      ...
      {tab==='统计'&&<OrdersManualQuery invoke={invoke} roster={roster} onError={setError}/>} 
...
function GlobalFilters({value,onChange,onQuery,onReset,options,meta}){const set=(k,v)=>onChange({...value,[k]:v});return <div className="rp-filterbar"><input className="rp-search" value={value.q} onChange={e=>set('q',e.target.value)} onKeyDown={e=>{if(e.key==='Enter')onQuery()}} placeholder="搜索姓名 / ID / 团队 / 组别 / 岗位 / 负责人 / 工作内容"/><Select value={value.shift} onChange={v=>set('shift',v)} options={options.shifts} all="全部班次"/><Select value={value.team} onChange={v=>set('team',v)} options={options.teams} all="全部团队"/><Select value={value.group} onChange={v=>set('group',v)} options={options.groups} all="全部组别"/><Select value={value.position} onChange={v=>set('position',v)} options={options.positions} all="全部岗位"/><Select value={value.country} onChange={v=>set('country',v)} options={options.countries} all="全部国家"/><input list="rp-supervisors" value={value.supervisor} onChange={e=>set('supervisor',e.target.value)} onKeyDown={e=>{if(e.key==='Enter')onQuery()}} placeholder="负责人 / 培训 / 组长"/><datalist id="rp-supervisors">{(options.supervisors||[]).map(x=><option key={x} value={x}/>)}</datalist><Select value={value.platform} onChange={v=>set('platform',v)} options={options.platforms} all="全部盘口"/><button className="rp-query" onClick={onQuery}>查询</button><button className="rp-reset" onClick={onReset}>重置</button><div className="rp-filter-meta">{meta}</div></div>}
...
function Platforms({rows}){const [modal,setModal]=useState(null),total=uniqueCount(rows);const list=useMemo(()=>{const m=new Map();rows.forEach(r=>{const pan=text(r.platform)||'未填写',name=text(r.name);if(!name)return;if(!m.has(pan))m.set(pan,{rows:[],names:new Set(),day:new Set(),night:new Set(),mid:new Set(),pos:new Map()});const x=m.get(pan);x.rows.push(r);x.names.add(name);if(text(r.shift).includes('Day'))x.day.add(name);if(text(r.shift).includes('Night'))x.night.add(name);if(text(r.shift).includes('中'))x.mid.add(name);const p=text(r.position)||'未填写';if(!x.pos.has(p))x.pos.set(p,new Set());x.pos.get(p).add(name)});return [...m].map(([pan,x])=>({pan,rows:x.rows,total:x.names.size,day:x.day.size,night:x.night.size,mid:x.mid.size,pos:[...x.pos].map(([name,s])=>({name,count:s.size})).sort((a,b)=>b.count-a.count)})).sort((a,b)=>b.total-a.total)},[rows]);return <section className="rp-card"><div className="rp-card-title"><div><h2>盘口人数统计</h2><p>与原版相同：按盘口单元格原值分组，人数按姓名去重。</p></div><span>总人数 {total}</span></div><div className="rp-table-scroll"><table className="rp-table rp-platform-table"><thead><tr><th>Rank</th><th>盘口</th><th>总人数</th><th>占比</th><th>白班</th><th>夜班</th><th>中班</th><th>岗位分布</th></tr></thead><tbody>{list.map((x,i)=><tr key={x.pan} className={i<3?'rp-top-row':''}><td>#{i+1}</td><td><button className="rp-link rp-pan" onClick={()=>setModal({title:`盘口：${x.pan}`,rows:x.rows})}>{x.pan}</button></td><td>{x.total}</td><td>{fmtPct(x.total,total)}</td><td>{x.day}</td><td>{x.night}</td><td>{x.mid}</td><td className="rp-wrap">{x.pos.map(p=><div key={p.name}>{p.name}: {p.count}</div>)}</td></tr>)}</tbody></table></div>{modal&&<RosterModal title={modal.title} rows={modal.rows} onClose={()=>setModal(null)}/>}</section>}
...
function OrdersManualQuery({invoke,roster,onError}){
  ...
  const rows=useMemo(()=>{
    let next=(data?.rows||[]).filter(r=>allowed.has(r.employee_id)&&(!appliedPosition||r.position===appliedPosition))
    return [...next].sort((a,b)=>{const av=Number(a[sort.key]||0),bv=Number(b[sort.key]||0);return sort.asc?av-bv:bv-av})
  },[data,allowed,appliedPosition,sort])
  useEffect(()=>setPage(1),[appliedPosition,sort,size,roster])
  ...
  const quick=kind=>{
    ...
    setRange(next);load(next,position)
  }
  const reset=()=>{const next={from:'',to:''};setRange(next);setPosition('');load(next,'')}
  ...
  return <section className="rp-card"><div className="rp-card-title"><div><h2>员工订单处理统计</h2><p>与原版同源：效率表「网站数据」（由 工作表4 + 填表 生成）+ 居家排班表「账号」做后台账号 → ID 映射。</p></div><span>{data?`${data.from||'—'} ~ ${data.to||'—'}`:'读取中'}</span></div><div className="rp-order-toolbar"><label>日期起<input type="date" value={range.from} onChange={e=>setRange({...range,from:e.target.value})}/></label><label>日期止<input type="date" value={range.to} onChange={e=>setRange({...range,to:e.target.value})}/></label><Select value={position} onChange={setPosition} options={data?.options?.positions||[]} all="全部岗位"/><button className="primary" disabled={loading} onClick={()=>load({...range},position)}>{loading?'查询中…':'查询'}</button><button onClick={reset}>重置</button><button onClick={()=>quick('7d')}>最近7天</button><button onClick={()=>quick('month')}>本月</button><button onClick={()=>quick('all')}>全部</button></div>{loading&&!data?<div className="rp-loading-inline">读取订单统计…</div>:<><div className="rp-table-scroll rp-order-scroll"><table className="rp-table rp-order-table"><thead><tr><th>入职日期</th><th>ID</th><th>姓名</th><th>团队</th><th>班次</th><th>国家</th><th>岗位</th><th>盘口</th><th><button onClick={()=>changeSort('total')}>总 ⇅</button></th><th><button onClick={()=>changeSort('avg')}>平均每天处理 ⇅</button></th><th><button onClick={()=>changeSort('mistake_count')}>错误次数 ⇅</button></th>{dates.map(d=><th key={d}>{d}<br/>成功/驳回</th>)}</tr></thead><tbody>{slice.map(r=><tr key={r.employee_id}><td>{r.hire_date||'—'}</td><td><strong>{r.employee_id}</strong></td><td>{r.name}</td><td>{r.team||'—'}</td><td>{r.shift||'—'}</td><td>{r.country||'—'}</td><td>{r.position||'—'}</td><td className="rp-pan-cell">{r.platform||'—'}</td><td><strong>{r.total}</strong></td><td>{r.avg}</td><td><button className="rp-link" onClick={()=>openMistakes(r.employee_id)}>{r.mistake_count}</button></td>{dates.map(d=>{const day=r.daily?.[d]||{success:0,reject:0},before=r.hire_date&&d<r.hire_date;return <td key={d}>{before?'0 / 0':`${day.success||0} / ${day.reject||0}`}</td>})}</tr>)}</tbody></table></div><Pagination page={page} pages={pages} total={rows.length} pageSize={size} loading={loading} onPage={setPage} onPageSize={n=>{setSize(n);setPage(1)}}/></>}{mistakes&&<MistakeListModal id={mistakes.id} rows={mistakes.rows} onClose={()=>setMistakes(null)}/>}</section>
}
