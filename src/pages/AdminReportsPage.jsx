...
const VISIBLE_OPS=OPS.filter(tab=>tab!=='排班表')
...
const errorGradeLabel=(value,locale='zh')=>{
  ...
  if(locale==='en')return({excellent:'Excellent',normal:'Normal',attention:'Attention',watch:'Priority',high:'High frequency'}[key]||'Excellent')
  return({excellent:'优秀',normal:'正常',attention:'注意',watch:'重点',high:'高频'}[key]||'优秀')
}
...
const personKey=rosterPersonKey
const uniqueCount=uniqueRosterCount
...
const reportErrorMessage=(error,fallback)=>{
  const message=readableErrorMessage(error)
  return !message||message==='操作失败'||/^\[object (?:Object|Array)\]$/i.test(message)?fallback:message
}
async function functionErrorDetail(error,fallback='请求失败'){
  ...
  const payloadMessage=payload?reportErrorMessage(payload,''):''
  const base=payloadMessage||(status===401?'登录已失效，请重新登录':status===403?'当前账号没有后台访问权限':status>=500?'服务暂时不可用，请稍后重试':await edgeFunctionErrorMessage({error,fallback}))
...
async function invokeErrorReport(body,allowSessionRefresh=true){
  ...
  if(data?.error){
    ...
    const message=reportErrorMessage(data,'错误统计读取失败')
...
export default function AdminReportsPage(){
  ...
  const load=async(silent=false,announceOperation='')=>{
    ...
    try{
      const actionByTab={总汇:'overview',人员:'people',排班表:'legacy_schedule',盘口人数:'platform',统计:'statistics_context'}
      const result=await invoke({action:actionByTab[tab]||'overview'})
...
function Loading(){return <div className="rp-card rp-loading">正在读取…</div>}
...
function Overview({data,rows}){
  ...
  const metricsReady=data?.order_metrics_status==='ok'
...
function Platforms({rows}){
  ...
  const list=useMemo(()=>{const groups=new Map();rows.forEach(row=>{const platform=text(row.platform)||'未填写',identity=personKey(row);if(!identity)return;if(!groups.has(platform))groups.set(platform,{rows:[],people:new Set(),day:new Set(),night:new Set(),mid:new Set(),positions:new Map()});const item=groups.get(platform);item.rows.push(row);item.people.add(identity);const shift=text(row.shift).toLowerCase();if(shift.includes('day')||shift.includes('白'))item.day.add(identity);else if(shift.includes('night')||shift.includes('夜'))item.night.add(identity);else if(shift.includes('mid')||shift.includes('中'))item.mid.add(identity);const position=text(row.position)||'未填写';if(!item.positions.has(position))item.positions.set(position,new Set());item.positions.get(position).add(identity)});return [...groups].map(([platform,item])=>({platform,rows:item.rows,total:item.people.size,day:item.day.size,night:item.night.size,mid:item.mid.size,positions:[...item.positions].map(([name,people])=>({name,count:people.size})).sort((a,b)=>b.count-a.count)})).sort((a,b)=>b.total-a.total)},[rows])
  const largest=list[0],missing=list.find(item=>item.platform==='未填写')?.total||0
  return <><section className="rp-card rp-platforms"><div className="rp-card-title"><div><h2>盘口人数统计</h2><p>员工 ID 优先去重，缺少 ID 时按姓名兜底；点击任一盘口卡片可查看完整员工名单。</p></div><span>{list.length} 个盘口</span></div><div className="rp-platform-kpis"><div><span>筛选后总人数</span><strong>{total}</strong><small>当前筛选范围</small></div><div><span>盘口类别</span><strong>{list.length}</strong><small>包含未填写</small></div><div><span>最大盘口</span><strong>{largest?.total||0}</strong><small>{largest?.platform||'—'}</small></div><div className={missing?'warn':''}><span>未填写盘口</span><strong>{missing}</strong><small>{missing?'建议补充资料':'资料完整'}</small></div></div><div className="rp-platform-grid">{list.map((item,index)=><button className={`rp-platform-card ${index<3?'is-top':''}`} key={item.platform} onClick={()=>setModal({title:`盘口：${item.platform}`,rows:item.rows})}><header><span className="rp-platform-rank">#{index+1}</span><strong title={item.platform}>{item.platform}</strong><em>{fmtPct(item.total,total)}</em></header><div className="rp-platform-total"><strong>{item.total}</strong><span>人</span></div><div className="rp-platform-progress"><i style={{width:`${Math.min(100,item.total/Math.max(1,largest?.total||1)*100)}%`}}/></div><div className="rp-platform-shifts"><span>白班 <b>{item.day}</b></span><span>夜班 <b>{item.night}</b></span><span>中班 <b>{item.mid}</b></span></div><div className="rp-platform-positions">{item.positions.slice(0,4).map(position=><span key={position.name}>{position.name} <b>{position.count}</b></span>)}{item.positions.length>4&&<span>+{item.positions.length-4} 岗位</span>}</div><footer>查看员工名单 →</footer></button>)}</div>{!list.length&&<div className="rp-empty">当前筛选条件下没有盘口人员</div>}</section>{modal&&<PlatformRosterModal title={modal.title} rows={modal.rows} onClose={()=>setModal(null)}/>}</>
}
...
function Errors({onError,notify}){
  ...
  return <section className="rp-card rp-native-errors-card" aria-busy={loading}>
    ...
    {loading&&data&&<div className="rp-searching-state" role="status" aria-live="polite"><i/><span>正在查询…</span></div>}
    {loading&&!data?<div className="rp-loading-inline">正在读取…</div>:<><div className="rp-table-scroll rp-errors-scroll"><table className="rp-table rp-errors-table" data-native-errors-v2723="1"><thead><tr><th>等级</th><SortTh field="employee_id">员工ID</SortTh><SortTh field="name">姓名</SortTh><SortTh field="employee_status">状态</SortTh><SortTh field="team">团队</SortTh><SortTh field="position">岗位</SortTh><SortTh field="platform">盘口</SortTh><SortTh field="error_type">错误类型</SortTh><SortTh field="score">扣分</SortTh><SortTh field="qc_person">质检人</SortTh><SortTh field="qc_date">质检时间</SortTh><SortTh field="leader_review">小组长复审</SortTh><SortTh field="qc_result">质检人对/错</SortTh><SortTh field="review_date">复检时间</SortTh><th>操作</th></tr></thead><tbody>{rows.map(r=>{const grade=errorGradeLabel(r.risk_level,locale);return <tr key={r.key}><td><button className="rp-error-grade rp-error-grade-action" data-grade={grade} title={`${r.employee_id} · 点击查看该员工全部错误记录`} onClick={()=>showEmployeeErrors(r.employee_id)}>{grade}</button></td><td>{canViewEmployeeDirectory?<button className="rp-link rp-employee-profile-link" onClick={()=>setEmployeeNo(r.employee_id)}>{r.employee_id}</button>:<strong>{r.employee_id||'—'}</strong>}</td><td>{r.name||'—'}</td><td><span className="rp-employee-status" data-status={text(r.employee_status)}>{employeeStatusName(r.employee_status)}</span></td><td>{r.team||'—'}</td><td>{r.position||'—'}</td><td><div className="rp-cell-clamp" title={r.platform}>{r.platform||'—'}</div></td><td><SourceValue value={r.error_type} clamp/></td><td><SourceValue value={r.score}/></td><td><SourceValue value={r.qc_person}/></td><td><SourceValue value={r.qc_date}/></td><td><SourceValue value={r.leader_review} clamp/></td><td><SourceValue value={r.qc_result} clamp/></td><td><SourceValue value={r.review_date}/></td><td><button className="rp-view-error" onClick={()=>setDetail(r)}>查看错误</button></td></tr>})}</tbody></table></div><Pagination page={page} pages={pages} total={total} pageSize={size} loading={loading} onPage={changePage} onPageSize={changeSize}/></>}
    {detail&&<ErrorDetailModal row={detail} onClose={()=>setDetail(null)}/>} 
    {canViewEmployeeDirectory&&employeeNo&&<ReportEmployeeDrawer employeeNo={employeeNo} onClose={()=>setEmployeeNo('')}/>}
  </section>
}
...
function ReportEmployeeDrawer({employeeNo,onClose}){
  ...
  return <div className="modal-mask detail-mask wfh-report-employee-mask" onMouseDown={onClose}><div className="employee-detail-drawer employee-detail-v12" onMouseDown={event=>event.stopPropagation()}>
    ...
    {state.loading?<div className="empty-state">读取完整员工档案...</div>:state.error?<div className="empty-state">{state.error}</div>:<>
      <div className="wfh-v2722-risk-summary" data-grade={grade} data-profile-metrics="1"><div className="risk-grade"><span>等级</span><strong>{grade}</strong></div><div><span>本月记录</span><strong>{Number(summary.month_records||0)} 笔</strong></div><div><span>总错误</span><strong>{Number(summary.total_errors||0)} 笔</strong></div><div><span>考试总次数</span><strong>{Number(summary.exam_attempts||0)} 次</strong></div><div><span>平均考试分数</span><strong>{summary.exam_average==null?'—':`${Number(summary.exam_average).toFixed(1)} 分`}</strong></div></div>
...
function PlatformRosterTable({rows}){return <div className="rp-table-scroll"><table className="rp-table rp-platform-roster-table"><thead><tr><th>团队</th><th>姓名</th><th>ID</th><th>国家</th><th>班次</th><th>岗位</th><th>盘口</th></tr></thead><tbody>{(rows||[]).map((r,i)=><tr key={r.key||`${r.employee_id}-${i}`}><td>{r.team||'—'}</td><td><strong>{r.name||'—'}</strong></td><td>{r.employee_id||'—'}</td><td>{r.country||'—'}</td><td>{r.shift||'—'}</td><td>{r.position||'—'}</td><td className="rp-pan-cell">{r.platform||'未填写'}</td></tr>)}</tbody></table></div>}
function PlatformRosterModal({title,rows,onClose}){return <Modal title={`${title}（${uniqueCount(rows)} 人）`} onClose={onClose} wide><PlatformRosterTable rows={[...(rows||[])].sort((a,b)=>text(a.team).localeCompare(text(b.team),'zh-CN')||text(a.position).localeCompare(text(b.position),'zh-CN')||text(a.name).localeCompare(text(b.name),'zh-CN'))}/></Modal>}
