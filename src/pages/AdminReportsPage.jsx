...
function GlobalFilters({value,onChange,options,meta}){const set=(k,v)=>onChange({...value,[k]:v});return <div className="rp-filterbar"><input className="rp-search" value={value.q} onChange={e=>set('q',e.target.value)} placeholder="Search name / ID / team / group / position / manager / 工作内容"/><Select value={value.shift} onChange={v=>set('shift',v)} options={options.shifts} all="全部班次"/><Select value={value.team} onChange={v=>set('team',v)} options={options.teams} all="全部团队"/><Select value={value.group} onChange={v=>set('group',v)} options={options.groups} all="全部组别"/><Select value={value.position} onChange={v=>set('position',v)} options={options.positions} all="全部岗位"/><Select value={value.country} onChange={v=>set('country',v)} options={options.countries} all="全部国家"/><input list="rp-supervisors" value={value.supervisor} onChange={e=>set('supervisor',e.target.value)} placeholder="负责人 / 培训 / 组长"/><datalist id="rp-supervisors">{(options.supervisors||[]).map(x=><option key={x} value={x}/>)}</datalist><Select value={value.platform} onChange={v=>set('platform',v)} options={options.platforms} all="全部盘口"/><button className="rp-reset" onClick={()=>onChange(blankFilters())}>重置</button><div className="rp-filter-meta">{meta}</div></div>}
...
function Errors({onError}){
  ...
  useEffect(()=>{const timer=setInterval(()=>{if(!document.hidden)load({silent:true})},300000);return()=>clearInterval(timer)},[range,filters,sort,page,size])
  ...
  const showEmployeeErrors=employeeId=>{const nextFilters={...blankErrorFilters(),employee_id:text(employeeId)},nextRange={from:'',to:''},nextSort={key:'qc_date',asc:false};setDetail(null);setEmployeeNo('');setFilters(nextFilters);setRange(nextRange);setSort(nextSort);setPage(1);load({nextFilters,nextRange,nextSort,nextPage:1})}
