...
function OrdersManualQuery({invoke,roster,allRoster,onError,filterValue,onFilterChange,onFilterQuery,onFilterReset,filterOptions,filterMeta}){
  ...
  const reset=()=>{const next={from:'',to:''};setRange(next);onFilterReset();load(next,{scope:allRoster})}
...
function SourceValue({value,clamp=false}){return text(value)?<div className={clamp?'rp-cell-clamp':''} title={text(value)}>{value}</div>:<span className="rp-source-missing" title="Google 源表对应单元格没有填写">源表未填写</span>}
function ErrorDetailModal({row,onClose}){return <Modal title={`${row.employee_id} · ${row.error_type||'错误记录'}`} onClose={onClose}><div className="rp-detail-grid"><Detail k="源表行号" v={row.source_row}/><Detail k="姓名" v={row.name}/><Detail k="团队" v={row.team}/><Detail k="岗位" v={row.position}/><Detail k="盘口" v={row.platform}/><Detail k="金额" v={row.amount}/><Detail k="扣分" v={row.score}/><Detail k="质检人" v={row.qc_person}/><Detail k="质检时间" v={row.qc_date}/><Detail k="复检时间" v={row.review_date}/><Detail k="会员/id /订单号" v={row.member_order} wide/><Detail k="错误备注" v={row.error_note} wide/><Detail k="正确操作方式" v={row.correct_action} wide/><Detail k="小组长复审 / 质检人对错" v={[row.leader_review,row.qc_result].filter(Boolean).join(' / ')} wide/>{row.source_missing_fields?.length>0&&<Detail k="源表未填写字段" v={row.source_missing_fields.join('、')} wide/>}</div></Modal>}
