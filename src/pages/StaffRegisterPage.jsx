...
export default function StaffRegisterPage(){
  ...
  if(result)return <AuthShell portal="staff" title="注册成功" subtitle="账号已绑定后台员工ID。"><div className="success-box"><div className="success-icon">✓</div><p><b>{result.employee_id}</b> · {result.employee_name}</p><Link className="primary-btn full" to="/staff/login">去登录</Link></div></AuthShell>
  return <AuthShell portal="staff" title="员工注册" subtitle="填写邮箱、密码和组长/助理发给您的专属激活码。"><form onSubmit={submit} className="auth-form"><label>登录邮箱<input type="email" value={f.email} onChange={e=>setF({...f,email:e.target.value})} required/></label><label>密码<input type="password" value={f.password} onChange={e=>setF({...f,password:e.target.value})} required/></label><label>确认密码<input type="password" value={f.confirm} onChange={e=>setF({...f,confirm:e.target.value})} required/></label><div className="password-rules">{checks.map(x=><div className={x.ok?'ok':'no'} key={x.label}>{x.ok?'✓':'○'} {x.label}</div>)}</div><label>员工激活码<input value={f.code} onChange={e=>setF({...f,code:e.target.value.toUpperCase()})} placeholder="A1B2C3-D4E5F6" required/></label><div className="helper">激活码自动匹配后台员工ID，无需自行填写员工ID。</div>{error&&<div className="error-box">{error}</div>}<button className="primary-btn full" disabled={!valid||loading}>{loading?'注册中...':'创建账号'}</button><div className="auth-links"><Link to="/staff/login">返回登录</Link></div></form></AuthShell>
}
